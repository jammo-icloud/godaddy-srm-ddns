...
- Logs IP changes, updates, errors, and daemon start/stop to
  `/var/packages/godaddy-ddns/target/var/godaddy-ddns.log`
  (auto-rotated at 256 KB). Unchanged-IP checks are silent by default;
  set `LOG_CHECKS="yes"` to log every 5-minute check as a heartbeat.
...
| `LOG_CHECKS` | `no` | Log only changes/errors (`no`) or every check (`yes`) |
